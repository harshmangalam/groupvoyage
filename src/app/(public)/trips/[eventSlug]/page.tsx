  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="pt-12 max-w-7xl  mx-auto px-4 py-8 md:py-12 flex flex-col gap-2">
        <Suspense>
          <TripDetailsWrapper paramsPromise={params} />
        </Suspense>
      </div>
    </div>
  );
...
async function TripDetailsWrapper({ paramsPromise }) {
  const { eventSlug } = await paramsPromise;
  const event = await getEventDetails({ eventSlug });
    ...
    <div>
      ...
      <div className="flex items-center mb-2 gap-1 flex-wrap mt-4">
        {event.categories.map((category) => (
          <Link href={`/trips/?categories=${category.slug}`} key={category.id}>
